...
class Tokenizer:
    def __init__(self, 
        ...
        if not vocab_output_path:
            ...
            _logger.debug(f"No output path provided. Fallback vocab to default path: {vocab_path}")
    ...
    def from_file(cls, config_path: str):
        ...
        # Note: we strip the strings back into integer tuples for merges
        instance = cls(
        ...
        # Reconstruct merges: "int, int" -> int
        instance.merges = {
            tuple(map(int, k.split(", "))): v 
            for k, v in config["merges"].items()
        }
        
        # Reconstruct vocab: int -> bytes
        # We start with base 256 and build up using the merges
...
if __name__ == "__main__":
    # Path to the file downloaded from VastAI
    config_path = "/Users/rcss/Documents/master/math-reasoning/data/vocab/tokenizer_config_20251202001900.txt"

    # Option A: Create a new instance directly
